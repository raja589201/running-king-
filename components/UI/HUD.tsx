...
import { GameStatus, GEMINI_COLORS, ShopItem, RUN_SPEED_BASE, COMBO_WINDOW_MS } from '../../types';
...
const ComboMeter: React.FC = () => {
    const { multiplier, multiplierEndTime } = useStore();
    const barRef = useRef<HTMLDivElement>(null);
    
    // Use ref for frame loop
    useEffect(() => {
        let afId: number;
        ...
        const updateBar = () => {
            if (barRef.current) {
                const now = Date.now();
                const remaining = Math.max(0, multiplierEndTime - now);
                const pct = (remaining / COMBO_WINDOW_MS) * 100;
                
                barRef.current.style.width = `${pct}%`;
                
                // Change color based on urgency
                if (pct < 25) barRef.current.style.backgroundColor = '#ff0055';
                else barRef.current.style.backgroundColor = '#00ffff';
            }
            afId = requestAnimationFrame(updateBar);
        };
    ...
    return (
        <div className="absolute top-20 left-4 md:left-8 flex flex-col items-start animate-in slide-in-from-left-5 fade-in duration-300">
             <div className="flex items-end space-x-2">
                <span className="text-4xl md:text-5xl font-black italic text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 to-red-500 font-cyber drop-shadow-[0_0_10px_rgba(255,165,0,0.5)]">
                    {multiplier}X
                </span>
                <span className="text-sm md:text-base font-bold text-white/80 mb-2 tracking-widest">COMBO</span>
             </div>
             <div className="w-full h-2 bg-gray-800/50 rounded-full mt-1 overflow-hidden w-32">
                 <div ref={barRef} className="h-full bg-cyan-400 shadow-[0_0_10px_#00ffff]" style={{ width: '100%' }} />
             </div>
        </div>
    );
};
    ...
    <div className={containerClass}>
        ...
        <div className="flex justify-between items-start w-full">
            <div className="flex flex-col">
                ...
                {/* COMBO METER (Visible when active) */}
                <ComboMeter />
            </div>
